...
3. In Implementation mode read [token-efficient-context.md](references/token-efficient-context.md), [project-memory.md](references/project-memory.md), and [project-memory-contract.md](references/project-memory-contract.md). Follow `read_order` (AGENTS → CURRENT_STATUS → PROGRESS → plan → TOOLING → DEBUGGING → git). Read status first and only the active ticket section of the plan. Do not re-plan completed scope. Reconcile, then `begin` before edits. A ticket is one coherent, independently verifiable outcome. Before first implementation run `../../scripts/pala_update.py check` (24h cache; never from hooks; offline-safe).
