...
class ReleaseTruthTests(unittest.TestCase):
    def test_truth_is_a_coherent_candidate_or_verified_public_release(self) -> None:
        ...
        self.assertEqual(truth["product_version"], "1.2.0")
        if truth["remote_publish"] == "passed":
            ...
            self.assertEqual(truth["last_published_version"], "1.1.2")
        else:
            self.assertIn(truth["build_release_state"], {"CONFIGURED_NOT_VERIFIED", "LOCAL RELEASE CANDIDATE VERIFIED"})
            self.assertEqual(truth["remote_observed_state"], "NOT PUBLISHED AS 1.2.0")
            self.assertEqual(truth["last_published_version"], "1.1.2")
            self.assertEqual(truth["remote_publish"], "not-run")
        self.assertEqual(truth["current_public_version"], "1.1.2")
    ...
    def test_matrix_and_drift_are_deterministic(self) -> None:
        ...
        expected_local = (
            "passed"
            if truth["build_release_state"] == "LOCAL RELEASE CANDIDATE VERIFIED"
            else "configured-not-verified"
        )
    ...
    def test_current_publication_matrix_has_no_required_version_drift(self) -> None:
        ...
        self.assertEqual(matrix["release_truth"]["product_version"], "1.2.0")
        self.assertEqual(matrix["release_truth"]["plugin_base_version"], "1.2.0")
        self.assertEqual(matrix["release_truth"]["current_public_version"], "1.1.2")
        self.assertEqual(matrix["release_truth"]["last_published_version"], "1.1.2")
