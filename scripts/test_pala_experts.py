...
class ExpertCommandTests(unittest.TestCase):
    ...
    def test_graphify_code_command_is_local_and_writes_outside_repo(self) -> None:
        command = self.experts.graphify_command(ROOT, ROOT.parent / "expert-data", semantic=False)
        self.assertEqual(command.args[:3], ("graphify", "extract", str(ROOT)))
    ...
    def test_graphify_semantic_command_forces_local_ollama(self) -> None:
        command = self.experts.graphify_command(ROOT, ROOT.parent / "expert-data", semantic=True)
    ...
    def test_serena_command_disables_dashboard_and_memories(self) -> None:
        command = self.experts.serena_command(ROOT, ROOT.parent / "serena-home")
    ...
    def test_codebase_memory_command_is_one_shot_and_root_bounded(self) -> None:
        command = self.experts.codebase_memory_command(ROOT, "index_repository")
        self.assertEqual(command.args[:3], ("codebase-memory-mcp", "cli", "index_repository"))
